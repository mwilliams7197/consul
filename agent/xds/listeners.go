...
func (s *Server) makePublicListener(cfgSnap *proxycfg.ConfigSnapshot, token string) (proto.Message, error) {
	...
	if l == nil {
		...
		filter, err := makeListenerFilter(
			false, cfg.Protocol, "public_listener", LocalAppClusterName, "", "", true, 0, "")
		if err != nil {
			return nil, err
		}
...
func (s *Server) makeUpstreamListenerIgnoreDiscoveryChain(
	...
	filter, err := makeListenerFilter(
		false, cfg.Protocol, upstreamID, clusterName, u.DestinationName, "", false, cfg.TimeoutMs, u.DestinationName)
	if err != nil {
		return nil, err
	}
...
func (s *Server) makeUpstreamListenerForDiscoveryChain(
	...
	filter, err := makeListenerFilter(
		useRDS, proto, upstreamID, clusterName, "upstream_", "", false, cfg.TimeoutMs, u.DestinationName)
	if err != nil {
		return nil, err
	}
...
func makeListenerFilter(
	useRDS bool,
	protocol, filterName, cluster, statPrefix, routePath string, ingress bool, timeout int, serviceName string) (envoylistener.Filter, error) {
	switch protocol {
	case "grpc":
		return makeHTTPFilter(useRDS, filterName, cluster, statPrefix, routePath, ingress, true, true, timeout, serviceName)
	case "http2":
		return makeHTTPFilter(useRDS, filterName, cluster, statPrefix, routePath, ingress, false, true, timeout, serviceName)
	case "http":
		return makeHTTPFilter(useRDS, filterName, cluster, statPrefix, routePath, ingress, false, false, timeout, serviceName)
	case "tcp":
		fallthrough
	default:
		if useRDS {
